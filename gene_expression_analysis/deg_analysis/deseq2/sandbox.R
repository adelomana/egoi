...
gene_sets_dir = '/home/adrian/projects/hegoi/results/subsamples/DESeq2/filtered'
output_dir = '/home/adrian/projects/hegoi/results/subsamples/DESeq2/enrichment'

working_labels = list.files(gene_sets_dir, pattern='_A_|_B_|_E_')
working_labels = list.files(gene_sets_dir, pattern='_B_')
...
for (working_label in working_labels) {
  ...
  #genes = df$NCBI.gene..formerly.Entrezgene..ID
  genes = df$Gene.name
  cat(blue(paste('working with', working_label, 'with', length(genes), 'genes', sep=' ')), fill=TRUE)
  
  # enrichment1 = enrichPathway(genes, pvalueCutoff=0.05, readable=TRUE)
  # obj = pairwise_termsim(enrichment)
  # emapplot(obj)
  # dotplot(obj)
